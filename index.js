...
export const parseCommand = function(passedArguments) {
  ...
  .option(
    ...
    '-l, --level [level]',
    'The "lowest" WCAG level to test against',
    /^(A|AA|AAA)$/i,
    'A'
  ).option(
    '-c, --certainty [certainty]',
    'Level of certainty of Tenode results [0, 20, 40, 60, 80, 100]',
    /^(0|20|40|60|80|100)$/i,
    0,
    parseFloat
  ).option(
    ...
    '-p, --priority [priority]',
    'Filter for Tenode results, filtering out those with a lower priortiy',
    /^(0|20|40|60|80|100)$/i,
    0,
    parseFloat
  ).option(
    ...
    '-u, --user-agent [uaString]',
    'A user-agent to be supplied during Tenode tests'
  ).option(
    '-H, --view-port-height [viewPortHeight]',
    'The height of the headless browser viewport in Tenode tests',
    parseFloat
  ).option(
    '-W, --view-port-width [viewPortWidth]',
